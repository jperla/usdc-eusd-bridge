...
console.log('='.repeat(72));
console.log('  * THE PAYOUT VALUES ARE STILL THE RELAYER\'S WORD: the amount,');
console.log('    the token id, AND the beneficiary. The TxOut digest binds the');
console.log('    ENCRYPTED forms -- a Pedersen commitment plus a masked value,');
console.log('    and a ciphertext memo -- and nothing on chain opens them.');
console.log('    So a relayer holding one genuine, quorum-signed, provably');
console.log('    included return still names its value and its payee freely.');
console.log('    All three close together: they need the same shared secret the');
console.log('    recipient check already computes. Opening the amount must also');
console.log('    RECOMPUTE THE COMMITMENT and require equality -- unmasking');
console.log('    alone would let a large masked value be paired with a smaller');
console.log('    unrelated commitment.');
