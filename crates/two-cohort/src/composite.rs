...
use crate::{
    ceremony::{AuditedAddress, CeremonyId, ComponentReveal},
...
/// alone, and a ceremony root was not. A release path can refuse
...
pub enum Provenance {
    ...
    /// produced the root. No process held both components, and the shares live
    /// with the participants that generated them.
    Ceremony(CeremonyId),
}
...
impl CompositeSpend {
    ...
    ///     is [`Error::ViewKeyMismatch`] instead;
    ///   * `R`, the tx public key of an output OBSERVED on chain. The
    ///     depositor's `r` is the depositor's; unlike
    ///     [`CompositeSpend::simulate`] nothing here invents it.
    ///
    ...
    /// Whether this root came out of a ceremony or a dealer.
